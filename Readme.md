...
- 📁 **Dynamic Document Upload**: Add an interface on the frontend allowing users to upload new PDFs to the ingestion pipeline directly.
- 💾 **Database Memory Persistence**: Replace the simple in-memory session cache (`node-cache`) with a database store (like MongoDB or PostgreSQL) for full historical context tracking.
- 🎨 **Enhanced UI Layout**: Provide options to configure LLM parameters (e.g. temperature) and selection between multiple vector namespaces directly from the sidebar.
